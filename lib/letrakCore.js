...
letrak.minima = {};
letrak.minima.organismos = 'ΔΗΜΟΣ ΘΕΣΣΑΛΟΝΙΚΗΣ';
...
letrak.deltio.prototype.isProselefsi = function() {
	return (this.prosapoGet() === 'ΠΡΟΣΕΛΕΥΣΗ');
};

letrak.deltio.prototype.oxiProselefsi = function() {
	return !this.isProselefsi();
};

letrak.deltio.prototype.isApoxorisi = function() {
	return (this.prosapoGet() === 'ΑΠΟΧΩΡΗΣΗ');
};

letrak.deltio.prototype.oxiApoxorisi = function() {
	return !this.isApoxorisi();
};
...
letrak.parousia.prototype.orarioSet = function(orario) {
	delete this.orario;
	...
	if (!(orario instanceof letrak.orario))
	return this;
...
letrak.parousia.prototype.kataxorisiGet = function() {
	return this.kataxorisi;
};
...
letrak.parousia.prototype.isPiragmeno = function() {
	return (this.kataxorisiGet() === 'ΣΥΝΤΑΚΤΗΣ');
};

letrak.parousia.prototype.oxiPiragmeno = function() {
	return !this.isPiragmeno();
};
...
// Η μέθοδος "isozigioSet" δημιουργεί πεδίο που αφορά στη χρονική διαφορά
// συμβάντος προσέλευσης/αποχώρησης με την αντίστοιχη προβλεπόμενη (από το
// ωράριο) ώρα προσέλευσης/αποχώρησης. Η χρονική διαφορά υπολογίζεται και
// τίθεται σε λεπτά της ώρας. Για να λειτουργήσει ορθά η εν λόγω μέθοδος
// είναι απαραίτητο να «γνωρίζει» τα στοιχεία του σχετικού δελτίου το οποίο
// περνάμε ως παράμετρο.

letrak.parousia.prototype.isozigioSet = function(deltio) {
	this.pleoneli = undefined;

	// Πρέπει να έχουμε περάσει δελτίο (ως "deltio" object).

	if (!deltio)
	return this;

	// Πρέπει να υπάρχει καθορισμένο ωράριο για τον υπάλληλο.
	...
	// Πρέπει να υπάρχει καταγραφή συμβάντος προσέλευσης/αποχώρησης.
	...
	// Το δελτίο πρέπει να έχει καθορισμένη ημερομηνία.
	...
	// Ο τύπος του δελτίου πρέπει να είναι καθορισμένος (δελτίο
	...
	this.isozigio = letrak.isozigioCalc(imerominia, prosapo, orario, meraora);
	return this;
};

// Η μέθοδος "isozigioGet" επιστρέφει τα λεπτά χρονικής διαφοράς μεταξύ
// συμβάντος προσέλευσης/αποχώρησης και της αντίστοιχης προβλεπόμενης
// ώρας προσέλευσης/αποχώρησης του υπαλλήλου. Σε περίπτωση που η σχετική
// property δεν υπάρχει, τίθεται από την ίδια τη μέθοδο και τότε είναι
// απαραίτητο το δελτίο το οποίο πρέπει να έχουμε περάσει ως παράμετρο.

letrak.parousia.prototype.isozigioGet = function(deltio) {
	if (this.hasOwnProperty('isozigio'))
	return this.isozigio;

	this.isozigioSet(deltio);
	return this.isozigio;
};

// Η function "isozigioCalc" δέχεται ημερομηνία δελτίου, είδος δελτίου,
// ωράριο υπαλλήλου και timestamp συμβάντος και επιστρέφει το πλεόνασμα
// ή το έλλειμα χρόνου σε λεπτά της ώρας.

letrak.isozigioCalc = function(imerominia, prosapo, orario, meraora) {
	let ora;
	let prosimo;
...
// Η function "isozigio2hm" δέχεται λεπτά της ώρας που έχουν προκύψει ως
// διαφορά συμβάντος προσέλευσης/αποχώρησης με την αντίστοιχη προβλεπόμενη
// ώρα και επιστρέφει ένα string της μορφής "[+-]HhMm" όπου "H" είναι οι ώρες
// και "M" τα λεπτά που αντιστοιχούν, π.χ. για 63 λεπτά θα επιστραφεί "+1h3m",
// ενώ για -38 λεπτά θα επιστραφεί "-38m". Εφόσον περάσουμε και δεύτερη true
// παράμετρο τότε το επιστρεφόμενο string εμπεριέχει και HTML διαμορφώσεις.

letrak.isozigio2hm = (lepta, html) => {
	let s = '';
...
letrak.orario = function() {
	...
	// Αν υπάρχει ένα argument, τότε αυτό πρέπει να είναι string στη
	// μορφή "HH:MM-HH:MM" π.χ. "07:00-15:00".
	...
	// Εναλλακτικά μπορούμε να περάσουμε και δύο string arguments της
	// μορφής "HH:MM" έκαστο.
...
letrak.orario.defaultAsString = '07:00-15:00';
