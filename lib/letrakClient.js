...
letrak.toolbarXristisSetupYes = () => {
	...
	on('click', (e) => letrak.kodikosAlagi(e)));
...
letrak.kodikosAlagi = function(e) {
	e.stopPropagation();

	$.post({
		'url': '/letrak/lib/kodikosAlagi.php',
		'dataType': 'json',
		'success': (rsp) => letrak.kodikosAlagiExec(rsp),
		'error': (e) => {
			pnd.fyiError('Αδυναμία αλλαγής κωδικού');
			console.error(e);
		},
	});
...
letrak.kodikosAlagiExec = function(data) {
	if (data.hasOwnProperty('e'))
	return pnd.fyiError(data.e);

	let url = self.location.origin + '/kartel/isodos/index.php?' +
		'pubkey=' + data.k;
	window.open(url, 'kodikosAlagi');
}
